...
export function createInvoiceHtml(data: InvoiceData) {
  ...
  const qr = data.qrDataUrl ? `<div class="qr-wrap"><img src="${data.qrDataUrl}" alt="Payment QR" /><span>Scan to pay ${money(data.balanceDue)}</span></div>` : "";
