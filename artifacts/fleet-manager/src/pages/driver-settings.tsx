...
export default function DriverSettings() {
  ...
  const downloadDriverReport = async () => {
    ...
    const rows = logs.map((log) => `<tr><td>${log.date}</td><td>${state.vehicles.find((vehicle) => vehicle.id === log.vehicleId)?.name || "Vehicle"}</td><td>${log.description || "Work entry"}</td><td>₹${(log.amount || 0).toLocaleString("en-IN")}</td></tr>`).join("");
