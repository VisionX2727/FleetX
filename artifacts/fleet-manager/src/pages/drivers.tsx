      ...
      <section className="mx-5 mt-4 space-y-4 rounded-2xl border border-primary/30 bg-primary/5 p-4">
        ...
        {members.length > 0 ? <div className="space-y-2">
          {members.map((member) => <button type="button" key={member.id} onClick={() => setDetailDriverId(member.id)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-border bg-card p-3 text-left active:bg-muted">
            <span className="min-w-0"><strong className="block truncate">{member.profile.name || "Driver"}</strong><span className="text-xs text-muted-foreground">{member.profile.phone} • {member.profile.vehicleIds.length} vehicle(s)</span></span>
        ...
        {state.drivers.filter((driver) => driver.type === activeType).length === 0 ? (
          ...
          state.drivers.filter((driver) => driver.type === activeType).map(driver => (
            <button type="button" key={driver.id} onClick={() => setDetailDriverId(driver.id)} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-border bg-card p-4 text-left shadow-sm active:bg-muted">
              <span className="min-w-0"><strong className="block truncate text-lg">{driver.name}</strong><span className="mt-1 block truncate text-sm font-semibold text-muted-foreground">{driver.phone}{driver.vehicleId ? ` • ${state.vehicles.find((vehicle) => vehicle.id === driver.vehicleId)?.name || "Assigned vehicle"}` : ""}</span></span>
          ...
          {detailDriverId && (() => {
            ...
            return (
              <>
                 ...
                 {member && <div className="mb-4 rounded-xl bg-muted p-3 text-sm"><strong>{member.profile.status || "Active"}</strong><span className="ml-2 text-muted-foreground">{member.profile.vehicleIds.length} permanent vehicle(s)</span>{member.profile.documents.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{member.profile.documents.map((document) => <a key={document.id} href={document.dataUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-card px-2 py-1 text-xs font-bold text-primary">{document.name}</a>)}</div>}</div>}
                ...
                <div className="fm-stack">
                  ...
                  {allLogs.map((log) => <div className="fm-list-row" key={log.id}><div><strong>{log.date} • {state.vehicles.find((item) => item.id === log.vehicleId)?.name || "Vehicle"}</strong><small>{log.description} • {log.hours} hours{state.driverPays.some((pay) => pay.logIds?.includes(log.id)) ? " • Paid" : ""}</small></div><div className="fm-list-value text-primary">₹{(log.driverDailyRate ?? displayDriver.dailyRate).toLocaleString("en-IN")}</div></div>)}
